...
exports.addProduct = async (req, res, next) => {
  try {
    ...
    const ownerId = req.user?.id || req.admin?.id;
...
exports.getAdminProducts = async (req, res, next) => {
  try {
    ...
    const query = role === "seller" ? { createdBy: req.user.id } : {};
    const result = await Product.find(query).populate("reviews");
    res.status(200).json({
      success: true,
      data: result,
    });
...
exports.updateProduct = async (req, res, next) => {
  try {
    if (req.user?.role?.toLowerCase() === "seller") {
      ...
      if (!existing || !ownerId || ownerId !== req.user.id?.toString()) {
