 ...
 * @desc    Register a new user with role (buyer, seller, or admin)
 * @access  Public
 */
exports.registerWithRole = async (req, res, next) => {
  try {
    ...
    if (!validRoles.includes(userRole)) {
      return res.status(400).json({
        status: "fail",
        message: "Invalid role. Must be either 'buyer', 'seller', or 'admin'",
      });
    }
...
exports.updateUser = async (req, res, next) => {
  try {
    ...
    if (req.body.phone !== undefined) {
      // Set phone, but don't set contactNumber if phone doesn't validate as mobile
      user.phone = req.body.phone || null;
      // Only set contactNumber if phone is a valid mobile number
      if (req.body.phone && validator.isMobilePhone(req.body.phone)) {
        user.contactNumber = req.body.phone;
      } else if (req.body.phone) {
        // If phone is provided but not valid mobile, clear contactNumber
